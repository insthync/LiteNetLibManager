...
namespace LiteNetLibManager
{
    ...
    {
#if UNITY_EDITOR || UNITY_STANDALONE || !UNITY_2017_1_OR_NEWER
        ...
        public bool StartServer()
        {
            ...
            try
            {
                ...
                _nextConnectionId = 1;
                Logging.Log($"[WebSocketServer] Started on {_location}");
            }
            catch (Exception ex)
            {
                Logging.LogError($"[WebSocketServer] Unable to start server: {ex.Message}\n{ex.StackTrace}");
                return false;
            }
            return true;
#else
        ...
        private void _socket_OnError(Fleck.IWebSocketConnection conn, string message)
        {
            _eventQueue.Enqueue(new TransportEventData()
            {
                ...
                endPoint = new IPEndPoint(IPAddress.Parse(conn.ConnectionInfo.ClientIpAddress), conn.ConnectionInfo.ClientPort),
